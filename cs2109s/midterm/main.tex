...
% \begin{itemize}
%     \item Maximise margin between classification boundary and values
%     \item Define appropriate decision rule: $w\cdot x + b \geq 0$, then +
%     \item Find equation of margin $=\frac{2}{||w||}$ given by $\frac{w\cdot x^+-w\cdot x^-}{||w||}$
%     \item Derive maximisation of margin, $max_w\frac{2}{||w||} $ or $min_w||w||$, $\widehat{y}^{(i)}(w \cdot x^{(i)}+b)-1\geq 0$
%     \item $||u||$ is the magnitude of the vector, $u\cdot v = u^Tv=p||v||$ where $p$ is the projection of $u$ onto $p$
%     \item If data is not linearly separable, introduce slack variable $\xi^{(i)}$
%     \item $w\cdot x^++b \geq 1 -\xi^{(i)}$, aim for $min\frac{1}{2}||w||^2+C\sum_i\xi^{(i)}$ such that $\widehat{y}(w\cdot x^{(i)}+b)-1 \geq 0-\xi^{(i)}$
% \end{itemize}

% \subsection{Kernel Methods and Kernel Trick}
% \begin{itemize}
%     \item Rather than defining high degree polynomials, define polynomial with degree $d$ and $n^d$ terms as $\phi(u) \cdot \phi(v)=K(u,v)=(u\cdot v)^d$
%     \item Gaussian Kernel: $K(u,v)=\phi(u)\cdot\phi(v)=e^{-\frac{||u-v||^2}{2\sigma^2}}$
%     \item With kernel trick, do not need to compute transformed features explicitly, and can have SVM with infinite dimensional features
%     \item Compute new features based on proximity to landmarks
%     \item Generate similarities to landmarks then use logistic regression
%     \item Not all similarity functions yield valid kernels, need to satisfy Mercer's theorem
% \end{itemize}

% \subsection{Hyperparameter Tuning}
% \begin{itemize}
%     \item The expected error of a model is $J_D(h)=\frac{1}{N}\sum_{i=1}^Nerror(h(x^{(i)}),y^{(i)}), (x^{(i)},y^{(i)})\in D$
%     \item Pick hyperparameters, train model, then evaluate
%     \item Tuning Methods:
%     \begin{itemize}
%         \item Grid Search: Exhaustively try all possible hyperparameters
%         \item Random Search: Randomly select hyperparameters
%         \item Successive Halving: Use all possible hyperparameters but with reduced resources, increase resources with smaller set
%         \item Bayesian Optimisation: Estimate optimisation space of hyperparameters
%         \item Eovlutionary Algorithms: Use evolutionary algorithms to select a population
%     \end{itemize}
% \end{itemize}

% \section{Neural Networks}
% \begin{itemize}
%     \item Perceptron: $\widehat{y}=g(\sum_{i=1}^nw_ix_i)$ where $g$ is a non-linear activation function
%     \item Can add in bias term $w_0$
%     \item Activation Functions: Step, Sigmoid, tanh, ReLU
%     \item Perceptron Learning Algorithm:
%     \begin{itemize}
%         \item Initialise $w_i$
%         \item For each instance $i$ with features $x^{(i)}$, classify $\widehat{y}^{(i)}=sgn(w^Tx^{(i)})$
%         \item Select one misclassifed instance and update weights
%         \item Repeat until convergence or max iterations
%     \end{itemize}
%     \item Perceptron Update Rule: $w=w+\eta(y-\widehat{y})x$, $\eta$ is learning rate
%     \item When there is misclassification, $w'$ will be close to $\pi/2$
%     \item Perceptrons are less robust then SVMs, and cannot converge on non-linearly separable data
%     \item Gradient Descent: $w=w-\eta(\widehat{y}-y)\widehat{y}(1-\widehat{y})x$
%     \item A neural network is a multi-layer perceptron
% \end{itemize}

% \section{Backpropagation}
% \begin{itemize}
%     \item Layer Activation: $a=g(f(x)), f(x)=w\cdot x$, $a^{[l]}=g^{[l]}((w^{[l]})^Ta^{[l-1]})$ for layer $l$
%     \item Forward Propagation: $\widehat{y}(x)=g^{[l]}(f^{[l]}(....g^{[1]}(f^{[1]}(x^{[0]}))))$
%     \item Tensor: Multi-dimensional matrix containing elements of a single type
%     \item Matrix Form: $\widehat{y}'(w^{[l]})=a^{[l-1]}(\delta^{[l]})^T, \delta^{[l]}=g'^{[l]}(f^{[l]})W^{[l+1]}\delta^{[l+1]}$
%     \item Backward Propagation: $\widehat{y}'(w^{[l]})=a^{[l-1]}(\delta^{[l]})^T, \delta^{[l]}=[g'^{[l]}(f^{[l]})]\circ (W^{[l+1]}\delta^{[l+1]})$
%     \item Backpropagation efficiently computes gradient by avoiding duplicate calculations, not computing unnecessary intermediate values, and computing the gradient of each layer
%     \item Many libraries support automated differentiation for common functions
% \end{itemize}

% \section{Deep Learning}
% \begin{itemize}
%     \item Deep Neural Networks are neural networks with $\geq3$ layers
%     \item Convolution: Consider a group of pixels at at time rather than individually
%     \item Multiply by kernel map for group and sum to get feature value
%     \item $(f*g)(t)\int_{-\infty}^\infty f(x)g(x-t)dx$, $x[m,n]*h[m,n]=\sum_{j=-\infty}^\infty\sum_{i=-\infty}^\infty x[i,j]*h[m-i,n-j]$
%     \item Flip first then multiply and sum, if not we are doing 2D correlation
%     \item Padding: Losing pixels at edge of image, add border of zeroes
%     \item Stride: Computing adjacent filters might be slow, skip some steps, get a smaller feature map
%     \item Convolution Layer: $A^{[l]}=g^{[l]}(W^{[l]}*A^{[l-1]})$, weights are kernels, activation is a 3D matrix which is concatenation of feature maps
%     \item Hyperparameters: Number of kernels, kernel size, padding, stride
%     \item Pooling Layer: Downsamples feature maps, helps to train later kernels to detect higher-level features, reduces dimensionality
%     \item Softmax: $P(y=j)=\frac{e^{w_j}}{\sum_{i=0}^ke^{w_i}}$
%     \item Overfitting: Use regularisation, drop out some activations randomly, or perform early stopping on training
%     \item Vanishing Gradients: With back propagation, multiply many gradients approximately zero, weights don't change
%     \item Exploding Gradients: Gradients keep getting larger, causing gradient descent to diverge
%     \item Mitigate with proper weight initialisation, using non-saturating activation functions like ReLU, batch normalisation, gradient clipping
% \end{itemize}

% \section{Unsupervised Learning}
% \begin{itemize}
%     \item Clustering: Like classification, but groups are not known
%     \item K-Means: Randomly initialise $K$ centroids, then repeatedly get index of cluster centroids, then assign data points to nearest centroid
%     \item $x^{(i)}$: $i$th training data point
%     \item $\mu_k$: cluster centroid $k$
%     \item $c^{(i)}$: index of cluster centroid closest to $x^{(i)}$
%     \item $J(c^{(1)},...,c^{(m)},\mu_1,...,\mu_k)=\frac{1}{m}\sum_{i=1}^m||x^{(i)}-\mu_{c^{(i)}}||^2$
%     \item Try many times, and pick lowest loss
%     \item $K$-Medoids: For each cluster, pick the point with minimum average distance to all other data points in the cluster
%     \item Picking $K$: Elbow Method: look at graph of loss and find elbow point, Business Need
%     \item Hierarchical Clustering: Every data point is a cluster, repeatedly find pairs of points that are nearest to each other and not in same cluster and merge
%     \item Dendogram: Shows merging of clusters
%     \item Many options for computing similarity, but high space and time complexity
%     \item Dimensionality Reduction: Transforming a set of data points to another set of data points with fewer dimensions which captures most properties of it
%     \item Principal Component Analysis: Find plane to project points on which information is "preserved"
%     \item Singular Value Decomposition: $X=U\Sigma V^T$, $U$: left singular vectors, $\Sigma$: singular values, $V^T$: right singular values
%     \item PCA: $\Sigma=\frac{1}{m}\Sigma_{i=1}^m(x^{(i)})(x^{(i)})^T$, $U_{reduce}$ is the first $k$ vectors of $U$, $z^{(i)}=U_{reduce}^Tx^{(i)}$
%     \item PCA takes average squared projection error, and to pick $k$, choose minimum $k$ such that variance in data is mostly retained
%     \item PCA provides compression and visualisation
% \end{itemize}

% \section{AI and Ethics}
% \begin{itemize}
%     \item Bias: Reporting Bias, Selection Bias, Automation bias
%     \item Harms: Representation: Identity of marginalised group is denied in ways that reaffirm their marginalised position, Allocation: Members of a marginalised group are refused access or opportunities
%     \item Ethics: Remove or control sensitive fields in training, ensuring that outcomes are fair with respect to sensitive fields
%     \item Algorithmic Bias: People are more likely to assume that algorithms are objective or error-free, and are more likely to be implemented with no appeals process in place
%     \item Sharing Data: Transparency around collection of data and how it would be used, and proper management
% \end{itemize}
