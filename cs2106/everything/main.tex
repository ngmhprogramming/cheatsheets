...
% \begin{itemize}
%     \item Magnetic Disk: Seek (Change Track), Rotation (Change Sector)
%     \item Disk Scheduling: OS needs to schedule disk I/O requests due to seek and rotational latency
%     \item Reduce overall waiting time by reducing seeking time as rotational latency is hard to mitigate
%     \item I/O requests indicated by track number
%     \item FCFS, Shortest Seek First (SSF), SCAN / Elevator (Bi-Direction: Innermost to Outermost and Back / SCAN, 1-Direction: Outermost to Innermost/ C-SCAN)
%     \item Deadline: 3 queues for I/O requests, Sorted, Read FIFO, Write FIFO
%     \item noop (No Operation): No sorting
%     \item cfq (Completely Fair Queueing): Time slice and per-process sorted queues
%     \item bfq (Budget Fair Queueing / Multiqueue): Fair sharing based on number of sectors requested
% \end{itemize}

% \subsection{Case Studies}
% \begin{itemize}
%     \item Microsoft FAT File System, most prevalent as FAT32
%     \item File data allocated to a number of data blocks / clusters, with allocation info kept as linked list, data block pointers kept separately in file allocation table
%     \item File allocation table has one entry per data block / cluster, stores disk block information, OS will cache in RAM to faciliate linked list traversal
%     \item FAT entry contains either FREE code, Block number of next block, EOF code, BAD block
%     \item Directory represented as special type of file, root directory stored in special location
%     \item Directory entry is 32 bytes, containing name and other metadata
%     \item Use first disk block number stored in directory entry to find starting point of linked disk blocks
%     \item Use FAT to find out subsequent disk blocks, terminated by EOF
%     \item Use disk block number to perform actual disk access on data blocks
%     \item Extended-2 File System: Use by Linux
%     \item Disk space is split into blocks, corresponding to one or more disk sectors, grouped into block groups
%     \item Each file is described by single special structure known as index node (128 bytes), containing file metadata and data block addresses
%     \item Superblock: Describes whole file system, duplicated in each block group for redundancy
%     \item Group Descriptors: Describe each of block group, duplicated in each as well
%     \item Block Bitmap: Keep track of usage status of blocks in this block group
%     \item I-Node Bitmap: Keep track of the usage status of I-Nodes of this block group
%     \item I-Node Table: Array of I-Nodes, contains only I-Nodes of this block group
%     \item Multilevel Data Blocks: Allow larger file size
%     \item Multilevel index: Similar to multi-level paging, first level index block points to a number of second level index blocks
%     \item I-node has a combination of direct indexing and multi-level index scheme
%     \item Allows fast access to small file, and flexibility in handling huge file
%     \item Data blocks of a directory stores linked list of directory entries for file information within this directory
%     \item Each directory entry contains I-Node number for that file, size of entry, length of file name, type, and actual name
%     \item Given a pathname, read root directory I-Node, look at next part in filename and recursively retrieve new I-nodes
%     \item File Deletion: Remove directory entry from parent, update I-Node bitmap, Update Block bitmap
%     \item Hard Link: Create directory entry which uses same I-Node number
%     \item Symbolic Link: Creates new file with pathname of original, can be easily invalidated
% \end{itemize}

% \section{Shell Interpreter}
% \begin{itemize}
%     \item When user presses key, interrupt is triggered
%     \item Interrupt occurs, register and CPU state is saved, handler routine is performed, restore registers and CPU state, return from interrupt
%     \item After receiving command, interpreter forks with a system call, spawning child process which it waits for
% \end{itemize}
