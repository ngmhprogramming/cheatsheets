...
\begin{itemize}
    ...
    \item Thread Model: Describes security of a system by the class of attacks it can prevent
...
\begin{itemize}
    ...
    \item CTR (Counter Mode): Each block $x_i$ uses $IV+i$, stream cipher
...
\begin{itemize}
    ...
    \item Psuedorandom sequence is generated from secret key and IV
...
\begin{itemize}
    ...
    \item Suppose we have $M=1.17\cdot2^{64}$ messages and each has a value from $T=2^{128}$, the probability of collision is $1-e^{(-\frac{M^2}{2T})}$, basically half the bits, $M>1.17 \sqrt{T}$ gives $P>0.5$
...
\begin{itemize}
    ...
    \item Publicly Available Directory: Directory server which supports lookup, but server cannot verify that information is authentic
\end{itemize}
...
\begin{itemize}
    ...
    \item Without certificate: User has to take other user's public key and compare it with result of query with CA
...
\begin{itemize}
    ...
    \item Or in GCM, send $E_k(i||m)$
    \item SSL is the predecessor of TLS, HTTPS is built on top of TLS
    \item Forward Secrecy: Eavesdropper cannot recover plaintext from authenticated key exchange, messages protected by sequence key, and master key
\end{itemize}

\section{Network Security}
\subsection{Computer Networking}
\begin{itemize}
    \item Collection of interconnected devices
    \item Uses packet switching where messages are broken down into packets and routed via switches and routers
    \item Decide how to route messages using intermediate nodes, might need multiple hops
    \item Network is organised into layers
    \item Transport + IP Layer often treated as one, address is an IP address and port
    \item Man-In-The-Middle can see and modify data unit in a specific layer
\end{itemize}

\subsection{Name Resolution and Attacks}
\begin{itemize}
    \item DNS (Domain Name System): Resolves domain names into IP addresses
    \item Client queries DNS server who responds with a query ID and response
    \item DNS Spoofing: Attacker replies DNS query with another malicious IP address
    \item DNS server could be single point of failure
...
\begin{itemize}
    \item Attack on availability which disrupts normal functionality of a server or service
    ...
    \item Reflection Attack: DOS where attackers send requests to intermediate nodes, in turn sending overwhelming traffic to victim
    \item Amplification Attack: Intermediate nodes response is significantly larger than attacker's request, e.g. using DNS or NTP over UDP
    ...
    \item Broadcast is disabled by default to prevent this
\end{itemize}

\subsection{Securing Layers}
\begin{itemize}
    \item SSL/TLS sits on trasnport layer to protect data with encryption and MAC
    \item IPSec protects IP layer by securing network connections
    \item WPA2 protects between Link and Physical layer; data traveling between wireless device and access point
    \item VPN enables remote user to securely connect to a private network, establishing a tunnel
    \item Typically TLS/SSL + WPA2, IPSec is expensive and difficult
\end{itemize}
...
\begin{itemize}
    ...
    \item Typically has front end and back end firewall around DMZ
    \item Firewalls perform packet filtering which inspects every packet, typically TCP/IP header
    \item Deep Packet Inspection: Inspecting payload as well, not just headers
    \item Example rules are checking source IP with whitelists or blacklists
...
\begin{itemize}
    ...
    \item Can be reflection (non-persistent) or stored XSS, for example in a forum page, server should carry out input validation
    ...
    \item Click Fraud: Malicious actors click on ads to generate fake revenue or drain a competitors budget
\end{itemize}

\section{Call Stack}
\begin{itemize}
    \item Code is treated as data and both are stored in the same memory
    \item Program counter is a register that stores address of next instruction
    \item After an instruction is executed, processor fetches another instruction from memory, and PC is incremented
    \item Direct branch: PC replaced by constant value
    \item Indirect branch: PC replace by value fetched from memory
\end{itemize}

\subsection{Control Flow Integrity}
\begin{itemize}
    \item Attacker can compromise execution by modifying code in memory or modifying the address for indirect branching
    \item Can overwrite executing code with malicious code
    \item Or overwrite a piece of control flow information for either a direct or indirect jump (stack smashing)
\end{itemize}
...
\subsection{Unsafe Function e.g. printf}
\begin{itemize}
    \item Uses a format string and parameters
    \item When format specifier is found in format string, program fetches value of parameter and displays it, even if a second parameter is not given
    \item By calling printf with a user specified string, an attacker can inject format specifiers to extract information or cause a crash or modify memory content
\end{itemize}
...
\begin{itemize}
    ...
    \item Do not trust user input, use canonical representations
\end{itemize}
...
\begin{itemize}
    \item Incrementing beyond maximum value usually loops over to its minimum value
    \item Some invariants might not hold and can lead to vulnerabilities
\end{itemize}

\subsection{Code Injection}
\begin{itemize}
    \item Scripting languages automate execution, interpreted rather than compiled
    \item Many scripting languages allow scripts to be stored data
    \item The most well known is an SQL injection, where malicious users can write SQL inside an input field which causes it to be executed
\end{itemize}

\subsection{Undocumented Access Point / Easter Egg}
\begin{itemize}
...
\begin{itemize}
    ...
    \item Testing: White-box (access to source code), black-box (no access to source code), grey-box testing (combination)
...
\begin{itemize}
    ...
    \item Security Parameter: Boundary set up so that malicious activities outside it will not affect resources within it
    \item Design uses Principle of Least Privilege, Compartmentalisation, Defense in depth, and Segregation of duty
    \item Defense in Depth: Multiple layers to prevent single point of failure
    \item Segregation of Duty: Assign different components to different peoples
    ...
    \item Every object has an owner, access rights decided by owner (discretionary access control) or system-wide policies (mandatory access control)
\end{itemize}
...
\begin{itemize}
    ...
    \item Grouping determined by role of subject; collection of procedures
    \item Privilege is often called protection rings
...
\begin{itemize}
    ...
    \item File System Permission: Grouped into RWX, for OGW
...
\begin{itemize}
    ...
    \item Application granted elevated privilege to freely access a file and any user can invoke it
    \item Need to ensure that application only performs intended operation
...
\begin{itemize}
    ...
    \item If enabled, process effective UID is inherited from UID of executable's owner
